...
const chordproRenderer_1 = require("../../src/chordproRenderer");
function cssObjectToString(selectorPrefix, styleMap) {
...
function extractDirectives(chordPro) {
...
function pickFirst(directives, ...keys) {
...
function renderMetadataBlock(chordPro, song) {
...
function renderChordProHtml(chordPro) {
    const parser = new chordsheetjs_1.default.ChordProParser();
    const song = parser.parse(chordPro);
    const bodyHtml = (0, chordproRenderer_1.renderChordProBodyHtml)(chordPro);
    const metadataHtml = renderMetadataBlock(chordPro, song);
    return `${metadataHtml}<div class="rendered-song">${bodyHtml}</div>`;
}
function buildPdfDocument(contentHtml, columns) {
    const formatter = new chordsheetjs_1.default.HtmlDivFormatter();
    const formatterCss = cssObjectToString('.chordpro-sheet', formatter.defaultCss);
    return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
async function generatePdfBuffer(chordPro, columns = 1) {
