...
function renderChordOnlyLine(line: string): string {
...
function renderLyricLine(line: string): string {
  ...
  if (firstMatch && firstMatch.index !== undefined && firstMatch.index > 0) {
    ...
    if (prefix) {
      columns.push(
        `<div class="column"><div class="chord chord-empty"></div><div class="lyrics">${escapeHtml(prefix)}</div></div>`,
      );
    }
  }
  ...
  for (let index = 0; index < matches.length; index += 1) {
    ...
    columns.push(
      `<div class="column"><div class="chord">${escapeHtml(chord)}</div><div class="lyrics">${escapeHtml(lyric)}</div></div>`,
    );
    cursor = lyricEnd;
  }
